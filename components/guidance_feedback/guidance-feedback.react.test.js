...
test('Show the relevant form elements when click on NO or YES button', () => {
	const component = renderer.create(<GuidanceFeedback />);
	...
	// Click on NO button
	tree.children[1].children[1].props.onClick(submitEvent);

	tree = component.toJSON();
	expect(tree).toMatchSnapshot();

	// Click on YES button
	tree.children[1].children[2].props.onClick(submitEvent);
...
test('Hide the form and button elements on submit', () => {
	...
	tree.children[2].children[1].props.onClick(submitEvent);
...
test('Check for the absence of the Guidance Feedback component', () => {
	const component = shallow(<GuidanceFeedback />);
	expect(component.exists()).toBeTruthy();
});

// Unit testing
test('Test the methods', () => {
	const component = mount(<GuidanceFeedback />);
	...
	expect(
		typeof component.instance().buildGTMObj(obj) === 'object'
	).toBeTruthy();
	...
	expect(
		typeof component.instance().pushInDataLayer(obj) === 'object'
	).toBeTruthy();
});
